...
class general_model2:
    ...
    def load_data(self, arbiter_num, xor_num, feedforward_num, lightweight_num, interpose_num):
        ...
        for a in range(arbiter_num):
            ...
            arbiter_data, arbiter_data_label = arbiter_puf.load_data(68, 300, random_seed, random_num)
            puf_label = np.ones((300, 1))*(total_num)
        ...
        for x in range(xor_num):
            ...
            #print("xx")
            xor_puf = XOR_PUF()
            xor_data, xor_data_label = xor_puf.load_data(68, 300, random_xor_num, random_seed1[x], random_seed2[x], 
                                                         random_seed3[x], random_seed4[x], random_seed5[x], 
                                                         random_seed6[x], random_num)
            puf_label = np.ones((300, 1))*(total_num)
            total_num = total_num-1
        ...
        for f in range(feedforward_num):
            ...
            ff_data, ff_data_label = ff_puf.load_data(68, 300, random_xor_num, f1, f2, random_seed1[f], random_seed2[f], 
                                                         random_seed3[f], random_seed4[f], random_seed5[f], 
                                                         random_seed6[f], random_num)
            puf_label = np.ones((300, 1))*(total_num)
            total_num = total_num-1
