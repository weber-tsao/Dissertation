...
class RL:
    def __init__(self, n_action, n_feature,
        ...
        ## initializer
        self.w_initializer = tf.keras.initializers.TruncatedNormal(mean=0, stddev=0.3)
    ...
    def choose_action(self, observation):
        ...
        action_choose = []
        #print(observation[0])
        if observation[0][1] != 45 and observation[0][0] != 245:
            if (observation[0][0] + UNIT) == (5+UNIT*(self.state_num-1)) and (observation[0][1] + UNIT) == 45:
                action_choose.append(2)
                #print("straight")
            elif (observation[0][0] + UNIT) == (5+UNIT*(self.state_num-1)) and observation[0][1] == 45:
                action_choose.append(0)
                #print("cross up")
            else:
                # cross up
                if observation[0][1] > UNIT and observation[0][0] < (self.maze_w - 1) * UNIT:
                    action_choose.append(0)
                    #print("cross up")
                # cross down
                if observation[0][1] < (MAZE_H - 1) * UNIT and observation[0][0] < (self.maze_w - 1) * UNIT:
                    action_choose.append(1)
                    #print("cross down")
                # straight
                if observation[0][0] < (self.maze_w - 1) * UNIT:
                    action_choose.append(2)
                    #print("straight")
        else:
            action_choose = [0, 1, 2]
    ...
    def getNet(self, test_item):
        q_eval = self.eval_net.predict(test_item)
        return q_eval